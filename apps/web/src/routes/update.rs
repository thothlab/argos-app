...
//!
//! Channel selection: an optional `X-Argos-Channel` request header
...
const CHANNEL_HEADER: &str = "X-Argos-Channel";
    ...
    async fn unknown_channel_returns_400() {
        ...
        let res = h
            ...
            .oneshot(
                Request::builder()
                    .uri("/api/update/darwin-aarch64/0.1.0")
                    .header("X-Argos-Channel", "experimental")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
