...
//!
//! The service reads `$ARGOS_DATA_DIR/manifest.json` (a single
//! source-of-truth manifest covering every platform) and returns the
//! slice for `{target}` if the latest version is newer than
//! `{current}`. Same-or-newer current → 204 No Content.
...
mod tests {
    ...
    async fn write_manifest(h: &test_harness::Harness, version: &str) {
        ...
        platforms.insert(
            ...
            PlatformEntry {
                signature: "<sig>".into(),
                url: format!("https://example.test/releases/argos-{version}-arm64.dmg"),
            },
        );
        let m = ManifestFile {
        ...
        tokio::fs::write(h.state.data_dir.join("manifest.json"), json)
            .await
            .unwrap();
    }
