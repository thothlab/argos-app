...
pub fn asset_for_target(target: &str) -> Option<&'static str> {
    match target {
        "macos-aarch64" | "darwin-aarch64" => Some("Argos_0.1.3_aarch64.dmg"),
        "linux-x64" | "linux-x86_64" => Some("Argos_0.1.3_amd64.AppImage"),
        "windows-x64" | "windows-x86_64" => Some("Argos_0.1.3_x64-setup.exe"),
...
mod tests {
    ...
    async fn redirects_to_configured_base() {
        ...
        assert_eq!(
            loc,
            "https://example.test/releases/latest/download/Argos_0.1.3_aarch64.dmg"
        );
    }
