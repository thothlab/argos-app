//! Argos landing page + update manifest + crash collection service.
//!
//! One binary, one container, one job: be the public face of Argos
//! at `argos.thothlab.tech` during closed alpha.
//!
//! Routes:
...
//!
//! Persistent state lives in `$ARGOS_DATA_DIR` (default
//! `/var/lib/argos-web`) so a container restart doesn't lose crashes
//! or manifest.
...
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("argos_web=info,tower_http=info")),
        )
        .with_target(false)
        .init();
...
impl Config {
    pub fn from_env() -> anyhow::Result<Self> {
        let bind = env::var("ARGOS_BIND")
            .unwrap_or_else(|_| "127.0.0.1:8743".to_string())
            .parse()?;
        let data_dir = env::var("ARGOS_DATA_DIR")
            .unwrap_or_else(|_| "/var/lib/argos-web".to_string())
            .into();
        let docs_dir = env::var("ARGOS_DOCS_DIR")
...
fn build_router(state: AppState, config: &Config) -> Router {
    ...
    Router::new()
        ...
        .route("/argos-gpg.pub", get(routes::landing::argos_gpg_pub))
