...
Argos сейчас в **closed alpha (P1 MVP)** — v0.1.5 в проде, мейнтейнеры активно прокликивают и чинят баги. **External issues и PR welcome уже сейчас**, но имей в виду:

- Большие фичи (новый протокол, новый формат импорта, рефакторинг крейтов) — сначала открой issue для обсуждения, потом PR.
- Маленькие фиксы (опечатки, очевидные баги, UX-фидбек, доп. golden-тесты к существующим импортёрам) — PR без согласования ок.
- Багрепорты с реальным workspace + воспроизведением — самое полезное что можно дать.

Если ты пришёл сюда из публичного релиза и хочешь просто пользоваться — поставь [последнюю сборку](https://argos.thothlab.tech/), читать ниже не обязательно.
